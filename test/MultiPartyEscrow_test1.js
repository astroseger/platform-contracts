...
contract('MultiPartyEscrow', function(accounts) {
    ...
    it ("Test Simple wallet 1", async function()
       ...
       it ("closing transaction (first channel)", async function()
        {
            ...
            let sgn = await signFuns.waitSignedClaimMessage(accounts[0], escrow.address, 0, N1 - 1000);
        ...
        {
            //first we claim, and do not send back remaing funds!
            let sgn = await signFuns.waitSignedClaimMessage(accounts[4], escrow.address, 1, N1);
            let vrs = signFuns.getVRSFromSignature(sgn.toString("hex"));
            ...
            //try to use old signutature. We can actually can do it, but 0 funds will be transfered
            escrow.channelClaim(1, N1, N1, vrs.v, vrs.r, vrs.s, false, {from:accounts[6]})

            //make new signature with commulative sum
            let sgn2 = await signFuns.waitSignedClaimMessage(accounts[4], escrow.address, 1, N1 + N1 - 1000);
            let vrs2 = signFuns.getVRSFromSignature(sgn2.toString("hex"));

            await escrow.channelClaim(1, N1 * 2 - 1000, N1 * 2 - 1000, vrs2.v, vrs2.r, vrs2.s, true, {from:accounts[6]});
     ...
     it ("Close the third channel", async function()
        {
            //sign message by the privet key of accounts[4]
            let sgn = await signFuns.waitSignedClaimMessage(accounts[4], escrow.address, 2, 990);

            let vrs = signFuns.getVRSFromSignature(sgn.toString("hex"));

            //We try claim bigger amount
            testErrorRevert(escrow.channelClaim(2, 1000, 990, vrs.v, vrs.r, vrs.s, true, {from:accounts[7]}));

            // Claiming the normal amount (and send back funds)
            await escrow.channelClaim(2, 990, 990, vrs.v, vrs.r, vrs.s, true, {from:accounts[7]})
     ...
     it ("Open the fourh channel and close it by timeout", async function()
        ...
        it ("Open the fifth channel for actual claim lesser than planned amount", async function()
        {
            ...
            let sgn = await signFuns.waitSignedClaimMessage(accounts[4], escrow.address, currentChannelId, plannedAmount);
            let vrs = signFuns.getVRSFromSignature(sgn.toString("hex"));
...
//console.log("accountBal_7 - ", accountBal_7)
            ...
            const [sender_a, signer_a, recipient_a, groupId_a, debit_a, credit_a, expiration_a]
            ...
            assert.equal(debit_a.toNumber(), channelDepositAmount)
            assert.equal(credit_a.toNumber(), 0)
        ...
        it ("Open the eight channel for Claim signed by signer", async function()
        {
            ...
            let sgn = await signFuns.waitSignedClaimMessage(accounts[5], escrow.address, currentChannelId, plannedAmount);
            let vrs = signFuns.getVRSFromSignature(sgn.toString("hex"));
    ...
    it ("Check validity of the signatures with js-server part (claim)", async function()
        {
            //claim message
            let sgn = await signFuns.waitSignedClaimMessage(accounts[2], escrow.address, 1789, 1917);
            assert.equal(signFuns.isValidSignatureClaim(escrow.address, 1789, 1917, sgn, accounts[2]), true,   "signature should be ok")
            assert.equal(signFuns.isValidSignatureClaim(escrow.address, 1789, 1918, sgn, accounts[2]), false,  "signature should be false")
            assert.equal(signFuns.isValidSignatureClaim(escrow.address, 1941, 1917, sgn, accounts[2]), false,  "signature should be false")
            assert.equal(signFuns.isValidSignatureClaim(accounts[2],    1789, 1917, sgn, accounts[2]), false,  "signature should be false")
